...
export class ShowResultsComponent implements OnDestroy {
  ...
  constructor(
    ...
    private examServices: ExamAPIService,
    ...
    this.x = examServices
      .getbyID(parseInt(actRoute.snapshot.paramMap.get('examid') || '0'))
      .subscribe((e) => {
        this.exam = e.result;

        if ((this.userdegree / this.exam.finalDegree) * 100 < 50) {
          this.message =
            'Sorry You Have Failed the ' + this.exam.name + ' Test';
          this.show =
            'Your score is ' + this.userdegree + ' / ' + this.exam.finalDegree;
          this.grade = '';
        } else if (
          (this.userdegree / this.exam.finalDegree) * 100 >= 50 &&
          (this.userdegree / this.exam.finalDegree) * 100 < 60
        ) {
          this.message =
            'Congeratulations You Have Passed the ' + this.exam.name + ' Test';
          this.show =
            'Your score is ' + this.userdegree + ' / ' + this.exam.finalDegree;
          this.grade = 'Your Grade is Acceptable';
        } else if (
          (this.userdegree / this.exam.finalDegree) * 100 >= 60 &&
          (this.userdegree / this.exam.finalDegree) * 100 < 70
        ) {
          this.message =
            'Congeratulations You Have Passed the ' + this.exam.name + ' Test';
          this.show =
          ...
          (this.userdegree / this.exam.finalDegree) * 100 >= 70 &&
          (this.userdegree / this.exam.finalDegree) * 100 < 80
        ) {
          this.message =
            'Congeratulations You Have Passed the ' + this.exam.name + ' Test';
          this.show =
