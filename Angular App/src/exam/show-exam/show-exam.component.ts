...
export class ShowExamComponent implements AfterViewInit, OnDestroy {
  ...
  z: Subscription = new Subscription();
  ...
  constructor(
    ...
    private userexamServices: UserexamApiService
  ) {
    ...
    var interval = setInterval(() => {
      ...
      if (timeleft == 0) {
        ...
        for (let index = 0; index < this.exam.questions.length; index++) {
          if (
            (this.form as unknown as NgForm).form.value[index] ==
            this.exam.questions[index].correctAnswer
          ) {
            this.degree++;
          }
        }
        ...
        this.y = this.userexamServices.Add(userex).subscribe((e) => {
          if (e.statusCode == 200) {
            this.router.navigate([
              '/exam/showresult',
              this.degree * this.questiondegree,
              this.exam.id,
            ]);
          } else {
    ...
    window.onscroll = function () {
      var ele = document.getElementById('examdatatable') as HTMLElement;
